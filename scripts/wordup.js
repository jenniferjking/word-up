...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            console.log("Real word = " + theAnswer);
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        if (wordSubmission.isRealWord == true) {
            scoreChip.text(wordScore(wordSubmission.word)).attr("class", "tag tag-sm tag-primary");
        };

        if (wordSubmission.isRealWord == false) {
            scoreChip.text("X").attr("class", "tag tag-sm tag-danger");
        };
